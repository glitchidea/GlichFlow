...
def revenue_report(request):
    ...
    # İstatistikler (sadece tamamlanan projeler gelir olarak sayılır)
    total_revenue = sales.filter(status='completed').aggregate(total=Sum('final_price'))['total'] or 0
    total_projects = sales.count()
    completed_projects = sales.filter(status='completed').count()
    
    # Aylık gelir dağılımı (son 12 ay) - sadece tamamlanan projeler
    monthly_data = []
    ...
    for i in range(12):
        ...
        month_revenue = ProjectSale.objects.filter(
            seller=request.user,
            status='completed',  # Sadece tamamlanan projeler
