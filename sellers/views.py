...
from .forms import (
    ...
    SaleExtraServiceForm, AdditionalCostForm, PriceCalculatorForm, PaymentReceiptForm
)
...
def sale_detail(request, pk):
    ...
    payment_receipts = sale.payment_receipts.all()
    total_paid = sum(p.amount for p in payment_receipts if p.status == 'completed')
    remaining_amount = sale.final_price - total_paid
    payment_percentage = (total_paid / sale.final_price * 100) if sale.final_price > 0 else 0
...
def file_preview(request, pk):
    """TXT ve MD dosyalarını inline göster, DOCX/PPTX için metin çıkarımı dene."""
    file_obj = get_object_or_404(ProjectFile, pk=pk, uploaded_by=request.user)
    ...
    if ext in ['.txt', '.log', '.csv']:
        with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
            content = f.read()
        return render(request, 'sellers/files/preview_text.html', {
            'file': file_obj,
            'content': content,
            'syntax': 'plain'
        })
    ...
    if ext in ['.json']:
        try:
            ...
            # JSON parse edilemezse düz metin göster
            with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                content = f.read()
        return render(request, 'sellers/files/preview_text.html', {
            'file': file_obj,
    ...
    if ext in image_exts:
        return render(request, 'sellers/files/preview_media.html', {
            'file': file_obj,
            'media_type': 'image',
        })
    ...
    if ext in ['.md', '.markdown']:
        try:
            ...
            return render(request, 'sellers/files/preview_markdown.html', {
                'file': file_obj,
                'html': html,
                'raw': raw,
            })
    ...
    if ext in ['.docx']:
        try:
            ...
            return render(request, 'sellers/files/preview_text.html', {
                'file': file_obj,
                'content': text or '[Boş içerik] (DOCX)',
                'syntax': 'plain'
            })
        except Exception as e:
            return render(request, 'sellers/files/preview_unsupported.html', {
                'file': file_obj,
                'reason': f'DOCX okunamadı: {e}'
            })
    ...
    if ext in ['.pptx']:
        try:
            ...
            return render(request, 'sellers/files/preview_text.html', {
                'file': file_obj,
                'content': content or '[Boş içerik] (PPTX)',
                'syntax': 'plain'
            })
        except Exception as e:
            return render(request, 'sellers/files/preview_unsupported.html', {
                'file': file_obj,
                'reason': f'PPTX okunamadı: {e}'
            })

    return render(request, 'sellers/files/preview_unsupported.html', {
        'file': file_obj,
        'reason': 'Bu dosya türü için önizleme desteklenmiyor.'
    })
...
@login_required
@seller_required
def edit_extra_service(request, service_pk):
    """Ek hizmet düzenleme"""
    service = get_object_or_404(SaleExtraService, pk=service_pk, sale__seller=request.user)
    
    if request.method == 'POST':
        form = SaleExtraServiceForm(request.POST, instance=service)
        if form.is_valid():
            form.save()
            messages.success(request, 'Ek hizmet başarıyla güncellendi.')
            return redirect('sellers:sale_detail', pk=service.sale.pk)
        else:
            messages.error(request, 'Ek hizmet güncellenirken hata oluştu.')
    else:
        form = SaleExtraServiceForm(instance=service)
    ...
    return render(request, 'sellers/sales/edit_extra_service.html', context)


@login_required
@seller_required
def delete_extra_service(request, service_pk):
    """Ek hizmet silme"""
    service = get_object_or_404(SaleExtraService, pk=service_pk, sale__seller=request.user)
    sale = service.sale
    ...
    if request.method == 'POST':
        service.delete()
        messages.success(request, 'Ek hizmet başarıyla silindi.')
        return redirect('sellers:sale_detail', pk=sale.pk)
    ...
    return render(request, 'sellers/sales/delete_extra_service.html', context)


# Ek Maliyet Yönetimi
@login_required
@seller_required
def edit_additional_cost(request, cost_pk):
    """Ek maliyet düzenleme"""
    cost = get_object_or_404(AdditionalCost, pk=cost_pk, sale__seller=request.user)
    
    if request.method == 'POST':
        form = AdditionalCostForm(request.POST, instance=cost)
        if form.is_valid():
            form.save()
            messages.success(request, 'Ek maliyet başarıyla güncellendi.')
            return redirect('sellers:sale_detail', pk=cost.sale.pk)
        else:
            messages.error(request, 'Ek maliyet güncellenirken hata oluştu.')
    else:
        form = AdditionalCostForm(instance=cost)
    ...
    return render(request, 'sellers/sales/edit_additional_cost.html', context)


@login_required
@seller_required
def delete_additional_cost(request, cost_pk):
    """Ek maliyet silme"""
    cost = get_object_or_404(AdditionalCost, pk=cost_pk, sale__seller=request.user)
    sale = cost.sale
    ...
    if request.method == 'POST':
        cost.delete()
        messages.success(request, 'Ek maliyet başarıyla silindi.')
        return redirect('sellers:sale_detail', pk=sale.pk)
    ...
    return render(request, 'sellers/sales/delete_additional_cost.html', context)
...
def delete_customer(request, customer_id):
    ...
    if request.method == 'POST':
        # Müşteriye bağlı projeler var mı kontrol et
        related_projects = customer.projects.all()
        if related_projects.exists():
            messages.error(request, 'Bu müşteriye bağlı projeler bulunduğu için silinemez. Önce projeleri silin.')
            return redirect('sellers:customer_detail', pk=customer.id)
        
        customer.delete()
        messages.success(request, 'Müşteri başarıyla silindi.')
        return redirect('sellers:customer_list')
    
    context = {
        'customer': customer,
        'related_projects': customer.projects.all()
    }
    return render(request, 'sellers/customers/delete_customer.html', context)
...
def delete_sale(request, sale_id):
    """Proje satışı silme"""
    sale = get_object_or_404(ProjectSale, id=sale_id, seller=request.user)
    
    if request.method == 'POST':
        # Projeye bağlı dosyalar var mı kontrol et
        related_files = sale.project_files.all()
        if related_files.exists():
            messages.error(request, 'Bu projeye bağlı dosyalar bulunduğu için silinemez. Önce dosyaları silin.')
            return redirect('sellers:sale_detail', pk=sale.id)
        
        sale.delete()
        messages.success(request, 'Proje satışı başarıyla silindi.')
        return redirect('sellers:sale_list')
    
    context = {
        'sale': sale,
        'related_files': sale.project_files.all()
    }
    return render(request, 'sellers/sales/delete_sale.html', context)


# Payment Receipt Views
@login_required
@seller_required
def add_payment_receipt(request, sale_id):
    """Ödeme makbuzu ekleme"""
    sale = get_object_or_404(ProjectSale, id=sale_id, seller=request.user)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            payment_receipt.sale = sale
            payment_receipt.created_by = request.user
            payment_receipt.save()
            messages.success(request, 'Ödeme makbuzu başarıyla eklendi.')
            return redirect('sellers:sale_detail', pk=sale.id)
    else:
    ...
    return render(request, 'sellers/payments/add_payment_receipt.html', context)


@login_required
@seller_required
def edit_payment_receipt(request, receipt_id):
    """Ödeme makbuzu düzenleme"""
    payment_receipt = get_object_or_404(PaymentReceipt, id=receipt_id, created_by=request.user)
    
    if request.method == 'POST':
        form = PaymentReceiptForm(request.POST, request.FILES, instance=payment_receipt)
        if form.is_valid():
            form.save()
            messages.success(request, 'Ödeme makbuzu başarıyla güncellendi.')
            return redirect('sellers:sale_detail', pk=payment_receipt.sale.id)
    else:
    ...
    return render(request, 'sellers/payments/edit_payment_receipt.html', context)


@login_required
@seller_required
def delete_payment_receipt(request, receipt_id):
    """Ödeme makbuzu silme"""
    payment_receipt = get_object_or_404(PaymentReceipt, id=receipt_id, created_by=request.user)
    
    if request.method == 'POST':
        sale_id = payment_receipt.sale.id
        payment_receipt.delete()
        messages.success(request, 'Ödeme makbuzu başarıyla silindi.')
        return redirect('sellers:sale_detail', pk=sale_id)
    
    context = {
        'payment_receipt': payment_receipt
    }
    return render(request, 'sellers/payments/delete_payment_receipt.html', context)
